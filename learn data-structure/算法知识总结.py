...
判断一个字符串的最长子回文字符串
首先将字符串转换成这种模式
原字符串    aba    转换字符串    #a#b#a#
char数组 长度和上面的转换字符串长度相同  记录每个对应下标字符的回文半径 例如上面的b的回文半径为4 a的为2
index 每次pr更新之后也更新index index的值为本次的回文中心下标
pr 是右边界即将到达得点 一开始为-1

进行算法， 下面是这个算法的加速过程
    首先如果现在找到的对应点 i 在pr内部 那么通过index可以找到他的对称点，该i与对称点i'的最长回文字符串相等
        x' i' y' 因为i的最大回文范围没有增加说明x' != y' 那么同理  x = x'  y = y'  y i x 的 x != y 所以相等
    第二如果 
        x' i' y' x'在 L的左边    i的最长回文字符串为y i R  因为 y = y' 同时 y = y'这个时候如果i的回文字符串还想扩张的话
        那么pr也是可以扩张的因为这样的话 L-1 = R+1然而正好这个等式不成立所以长度为上面所说的
    第三如果压线
        这样的话pr内部的就不需要去验验超过pr的部分
"""
...
"""
栈
实现的例子就是相当于编译器计算式子  1+2*2*4-6
两个栈实现
遍历式子，数字进a栈 运算符进b栈，如果新来的运算符的优先级高于栈顶那么入栈，低于或者等于栈顶拿出栈顶的运算符和a栈的
前两个数进行计算直到当前运算符的优先级大于b栈栈顶
